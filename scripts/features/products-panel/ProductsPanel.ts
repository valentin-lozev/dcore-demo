class ProductsPanel implements dcore.Module {
	...
	init() {
		...
		let view = new View(this.sandbox.root);
	...
	moduleDidReceiveMessage(msg: dcore.Message) {
		switch (msg.type) {
			case "change-filter": this.presenter.changeFilter(msg["filter"]); break;
			case "search-performed": this.presenter.search(msg["query"]); break;
			case "search-reset": this.presenter.reset(); break;
		}
	}
}